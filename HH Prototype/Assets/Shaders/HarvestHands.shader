...
Shader "GoblinHammerGames/HarvestHandsSF" {
    Properties {
        ...
        _ContrastLights ("Contrast Lights", Range(-1, 2)) = 1
        _ContrastDarks ("Contrast Darks", Range(-1, 1)) = 0
        [MaterialToggle] _TreeColourRandomization ("Tree Colour Randomization", Float ) = 0.147157
...
/////// Diffuse:
                ...
                float node_4425 = 0.0;
                float3 diffuseColor = (lerp(float3(1,1,1),saturate(3.0*abs(1.0-2.0*frac((lerp( _Hue, (sin(clamp(ColourRandomization,0.1,1))*0.3333333+-0.1333333), _TreeColourRandomization )+node_1229.r)+float3(0.0,-1.0/3.0,1.0/3.0)))-1),(_Sat+node_1229.g))*clamp((_Val+(_ContrastDarks + ( (node_1229.b - node_4425) * (_ContrastLights - _ContrastDarks) ) / (1.0 - node_4425))),0.0,1.0));
