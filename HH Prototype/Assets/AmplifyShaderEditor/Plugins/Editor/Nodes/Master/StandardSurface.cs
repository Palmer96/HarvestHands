...
namespace AmplifyShaderEditor
{
	...
	public sealed class StandardSurfaceOutputNode : MasterNode, ISerializationCallbackReceiver
	{
		...
		public override void AddMasterPorts()
		{
			...
			switch ( m_currentLightModel )
			{
				case StandardShaderLightModel.Standard:
				{
					...
					AddInputPort( WirePortDataType.FLOAT3, false, EmissionStr, index++ );
					AddInputPort( WirePortDataType.FLOAT, false, MetallicStr, index++ );
					AddInputPort( WirePortDataType.FLOAT, false, SmoothnessStr, index++ );
					AddInputPort( WirePortDataType.FLOAT, false, OcclusionStr, index++ );
				}
				break;
				case StandardShaderLightModel.StandardSpecular:
				{
					...
					AddInputPort( WirePortDataType.FLOAT3, false, NormalStr, 0 );
					AddInputPort( WirePortDataType.FLOAT3, false, EmissionStr, index++ );
					AddInputPort( WirePortDataType.FLOAT3, false, SpecularStr, index++ );
					AddInputPort( WirePortDataType.FLOAT, false, SmoothnessStr, index++ );
					AddInputPort( WirePortDataType.FLOAT, false, OcclusionStr, index++ );
				}
				break;
				case StandardShaderLightModel.Lambert:
				{
					...
					AddInputPort( WirePortDataType.FLOAT3, false, NormalStr, 0 );
					AddInputPort( WirePortDataType.FLOAT3, false, EmissionStr, index++ );
					AddInputPort( WirePortDataType.FLOAT, false, SpecularStr, index++ );
					AddInputPort( WirePortDataType.FLOAT, false, GlossStr, index++ );
				}
				break;
			...
			AddInputPort( WirePortDataType.FLOAT3, false, TransmissionStr, index++ );
			...
			AddInputPort( WirePortDataType.FLOAT3, false, TranslucencyStr, index++ );
			m_inputPorts[ m_inputPorts.Count - 1 ].Locked = ( m_currentLightModel == StandardShaderLightModel.Standard ) || ( m_currentLightModel == StandardShaderLightModel.StandardSpecular ) ? false : true;

			AddInputPort( WirePortDataType.FLOAT, false, RefractionStr, index + 2 );
			...
			AddInputPort( WirePortDataType.FLOAT, false, AlphaStr, index++ );
			m_inputPorts[ m_inputPorts.Count - 1 ].DataName = AlphaDataStr;
			m_opacityPortId = m_inputPorts.Count - 1;
			m_inputPorts[ m_inputPorts.Count - 1 ].Locked = ( m_alphaMode == AlphaMode.Opaque || m_alphaMode == AlphaMode.Masked );

			AddInputPort( WirePortDataType.OBJECT, false, DiscardStr, index++ );
			...
			AddInputPort( WirePortDataType.FLOAT3, false, VertexDisplacementStr, index++, MasterNodePortCategory.Vertex );
			AddInputPort( WirePortDataType.FLOAT3, false, VertexNormalStr, index++, MasterNodePortCategory.Vertex );


			AddInputPort( WirePortDataType.OBJECT, false, CustomLightModelStr, index++ );
			m_inputPorts[ m_inputPorts.Count - 1 ].Locked = true;

			m_tessOpHelper.MasterNodeIndexPort = m_inputPorts.Count;
			AddInputPort( WirePortDataType.FLOAT4, false, TessellationOpHelper.TessellationPortStr, index++, MasterNodePortCategory.Tessellation );


			////////////////////////////////////////////////////////////////////////////////////
			AddInputPort( WirePortDataType.FLOAT3, false, DebugStr, index++ );
		...
		public void DrawRenderingPlatforms( GUIStyle toolbarstyle)
		{
			...
			UIUtils.CurrentWindow.ExpandedRendering = GUILayout.Toggle( UIUtils.CurrentWindow.ExpandedRendering, RenderingPlatformsStr, m_inspectorFoldoutStyle );
			if ( EditorGUI.EndChangeCheck() )
			{
				EditorPrefs.SetBool( "ExpandedRendering", UIUtils.CurrentWindow.ExpandedRendering );
			}
			EditorGUILayout.EndHorizontal();
			if ( UIUtils.CurrentWindow.ExpandedRendering )
			{
		...
		public override void DrawProperties()
		{
			...
			if ( m_inspectorToolbarStyle == null || EditorGUIUtility.isProSkin != m_usingProSkin )
			{
				...
				//m_inspectorToolbarStyle.margin.bottom = 2;
			}
			...
			EditorGUILayout.BeginVertical();
			{
				...
				EditorGUI.BeginChangeCheck();
				UIUtils.CurrentWindow.ExpandedMain = GUILayout.Toggle( UIUtils.CurrentWindow.ExpandedMain, " Main Properties", "foldout" );
				if ( EditorGUI.EndChangeCheck() )
				{
				...
				if ( UIUtils.CurrentWindow.ExpandedMain )
				{
					//cachedColor = GUI.color;
				...
				GUI.color = m_cachedColor;
				EditorGUI.BeginChangeCheck();
				UIUtils.CurrentWindow.ExpandedBlend = GUILayout.Toggle( UIUtils.CurrentWindow.ExpandedBlend, AlphaModeContent, "foldout", GUILayout.Width( m_textLabelWidth - 8 ) );
				if ( EditorGUI.EndChangeCheck() )
				{
				...
				EditorGUI.BeginChangeCheck();
				m_alphaMode = ( AlphaMode )EditorGUILayout.Popup( "", ( int )m_alphaMode, FadeModeOptions, m_inspectorPopdropdownStyle, GUILayout.Width( 100 ), GUILayout.Height( 19 ) );
				if ( EditorGUI.EndChangeCheck() )
				{
				...
				if ( UIUtils.CurrentWindow.ExpandedBlend )
				{
					GUI.color = new Color( m_cachedColor.r, m_cachedColor.g, m_cachedColor.b, m_boxOpacity );
					EditorGUILayout.BeginVertical( "TE NodeBackground" );
					GUI.color = m_cachedColor;
					...
					EditorGUI.BeginDisabledGroup( !(m_alphaMode == AlphaMode.Fade || m_alphaMode == AlphaMode.Transparent ||  m_alphaMode == AlphaMode.Translucent  || m_alphaMode == AlphaMode.Custom) );
				...
				DrawRenderingPlatforms( m_inspectorToolbarStyle );
				DrawMaterialInputs( m_inspectorToolbarStyle );

				//m_inspectorPopdropdownStyle.alignment = TextAnchor.UpperRight;
				//m_inspectorPopdropdownStyle.border.bottom = 0;

				//m_inspectorTooldropdownStyle.fixedHeight = cachedHeight;
				//m_inspectorTooldropdownStyle.margin.bottom = cachedmargin;

				//m_inspectorToolbarStyle.margin.bottom = cachedmargin;
				//m_inspectorToolbarStyle.fixedHeight = cachedHeight;
				//DrawShaderKeywords();
				//EditorGUILayout.Separator();
		...
		public void AddLocalVarInstructions( bool normalIsConnected )
		{
			if ( normalIsConnected )
				UIUtils.CurrentDataCollector.DirtyNormal = true;
			...
			for ( int i = 0; i < count; i++ )
			{
				bool isVertex = UIUtils.GetCategoryInBitArray( sortedList[ i ].Category, MasterNodePortCategory.Vertex ) || UIUtils.GetCategoryInBitArray( sortedList[ i ].Category, MasterNodePortCategory.Tessellation );
		...
		public override Shader Execute( string pathname, bool isFullPath )
		{
			...
			AddLocalVarInstructions( normalIsConnected );
			...
			if ( m_inputPorts[ m_inputPorts.Count - 1 ].IsConnected )
			{
				...
				CreateInstructionsForPort( debugPort, Constants.OutputVarStr + ".Emission", false, null, null, UIUtils.IsNormalDependent(), normalIsConnected );
			}
			else
			{
				// Custom Light Model
			...
			OpenShaderBody( ref ShaderBody, m_shaderName );
			{
				...
				OpenSubShaderBody( ref ShaderBody );
				{
					...
					if ( count != m_colorMask.Length && m_customBlendAvailable )
					{
						...
						OpenCGProgram( ref ShaderBody );
					{
						...
						if ( hasRefraction )
						{
							...
							ShaderBody += "\t\t\t\tcolor = lerp( Refraction( " + Constants.InputVarStr + ", " + Constants.OutputVarStr + ", " + refractionCode + ", _ChromaticAberration ), color, " + Constants.OutputVarStr + ".Alpha );\n";
							ShaderBody += "\t\t\t#endif\n";
							ShaderBody += "\t\t}\n\n";
						}
					...
					if ( m_customShadowCaster )
					{
						...
						if( hasVertexOffset )
						{
							...
							ShaderBody += "\t\t\t\tvertexDataFunc( v, customInputData );\n";
						}
