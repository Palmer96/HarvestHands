...
namespace AmplifyShaderEditor
{
...

	{
		...
		public override string GenerateShaderForOutput( int outputId, ref MasterNodeDataCollector dataCollector, bool ignoreLocalvar )
		{
			...
			string vspeed = "float fbspeed" + m_uniqueId + " = _Time[1] * abs(" + speed + ");";
			...
			string vcurrenttileindex = "int fbcurrenttileindex" + m_uniqueId + " = (int)fmod( fbspeed" + m_uniqueId + " , fbtotaltiles" + m_uniqueId + ");";
			...
			if ( _negativeSpeedBehavior == 0 )
			{
				if ( _selectedTextureVerticalDirection == 0 )
				{
					...
					voffsety2 = "fblinearindextoy" + m_uniqueId + " = (int)" + rows + " - fblinearindextoy" + m_uniqueId + ";";
				}
			}
			else
			{
