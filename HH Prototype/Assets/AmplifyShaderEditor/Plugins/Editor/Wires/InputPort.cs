...
namespace AmplifyShaderEditor
{
	...
	public sealed class InputPort : WirePort
	{
		...
		public InputPort( int nodeId, int portId, WirePortDataType dataType, string name, bool typeLocked, int orderId = -1, MasterNodePortCategory category = MasterNodePortCategory.Fragment ) : base( nodeId, portId, dataType, name, orderId ) { m_dataName = name; m_typeLocked = typeLocked; m_category = category; UpdateInternalData(); }
		...
		{
			string label = ( useCustomLabel == true && customLabel != null ) ? customLabel : m_name;
			switch ( m_dataType )
			{
				case WirePortDataType.OBJECT:
				case WirePortDataType.FLOAT:
