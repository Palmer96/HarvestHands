...
namespace AmplifyShaderEditor
{
	...
	public class ParentGraph : ISerializationCallbackReceiver
	{
		...
		public ParentGraph()
		{
			...
			nodeStyleOff = UIUtils.CustomStyle( CustomStyle.NodeWindowOff );
			nodeStyleOn = UIUtils.CustomStyle( CustomStyle.NodeWindowOn );
			nodeTitle = UIUtils.CustomStyle( CustomStyle.NodeHeader );
			commentaryBackground = UIUtils.CustomStyle( CustomStyle.CommentaryBackground );
		}
		...
		Rect DrawBezier( float invertedZoom, Vector3 startPos, Vector3 endPos, WirePortDataType inputDataType, WirePortDataType outputDataType, WireStatus wireStatus, Texture2D wireTex, ParentNode inputNode = null, ParentNode outputNode = null )
		{
			...
			if ( inputNode != null && inputNode.GetType() == typeof( WireNode ) )
				endTangent = endPos + ( ( inputNode as WireNode ).TangentDirection ) * mag * 0.33f;

			if ( outputNode != null && outputNode.GetType() == typeof( WireNode ) )
			...
			if ( UIUtils.CurrentWindow.Options.ColoredPorts && wireStatus != WireStatus.Highlighted )
				return GLDraw.DrawBezier( startPos, startTangent, endPos, endTangent, UIUtils.GetColorForDataType( outputDataType, false, false ), UIUtils.GetColorForDataType( inputDataType, false, false ), wiresTickness * 0.7f, segments );
			else
				return GLDraw.DrawBezier( startPos, startTangent, endPos, endTangent, UIUtils.GetColorFromWireStatus( wireStatus ), wiresTickness * 0.7f, segments );
		...
		public void CreateConnection( int inNodeId, int inPortId, int outNodeId, int outPortId )
		{
			...
			if ( outputNode != null )
			{
				...
				if ( outputPort != null )
				{
					...
					InputPort inputPort = inputNode.InputPorts[ inPortId ];
			...
			if ( isInput )
			{
				...
				if ( inputPort.IsConnected )
				{
					...
					for ( int i = 0; i < inputPort.ExternalReferences.Count; i++ )
					{
						...
						outputNode.GetOutputPortById( inputReference.PortId ).InvalidateConnection( inputPort.NodeId, inputPort.PortId );
			...
			else
			{
				...
				if ( outputPort.IsConnected )
				{
					...
					for ( int i = 0; i < outputPort.ExternalReferences.Count; i++ )
					{
						...
						inputNode.GetInputPortById( outputReference.PortId ).InvalidateConnection( outputPort.NodeId, outputPort.PortId );
						if ( propagateCallback )
