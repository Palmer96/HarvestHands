...
public class AmplifyShaderEditorWindow : EditorWindow, ISerializationCallbackReceiver
{
	...
	public Shader m_previewShader;
	public Material m_previewMaterial;

	// Auto-Compile samples 
	private bool m_forcingShaderUpdateFlag = false;
	private bool m_forcingShaderUpdateOp = false;
	private List<Shader> m_shadersToUpdate = new List<Shader>();
	...
	public void OnEnable()
	{
		...
		{
			m_previewShader = Shader.Find( "PreviewNodes" );
	...
	void Destroy()
	{
		...
		m_shadersToUpdate.Clear();
		m_shadersToUpdate = null;
	...
	void OnLeftMouseDown()
	{
			...
			else
			{
				...
				if ( wireRefs != null && wireRefs.Count > 0 )
				{
					for ( int i = 0; i < wireRefs.Count; i++ )
					{
						...
						OutputPort outputPort = outNode.GetOutputPortById( wireRefs[ i ].OutPortId );
						InputPort inputPort = inNode.GetInputPortById( wireRefs[ i ].InPortId );
		...
		if ( !m_contextMenu.CheckShortcutKey() && m_currentEvent.modifiers != EventModifiers.Shift )
		{
												 ...
												 ( inputPort.TypeLocked && inputPort.DataType == UIUtils.OutputPortReference.DataType ) ) )
					{
						...
						if ( inputPort.IsConnected )
						{
							if ( m_currentEvent.control && UIUtils.SwitchPortReference.IsValid )
							{
								...
								OutputPort oldOutputPort = oldOutputNode.GetOutputPortById( inputPort.GetConnection( 0 ).PortId );

								ParentNode switchNode = UIUtils.GetNode( UIUtils.SwitchPortReference.NodeId );
								InputPort switchPort = switchNode.GetInputPortById( UIUtils.SwitchPortReference.PortId );
	...
	void OnKeyboardDown()
	{
		if ( Constants.DeveloperMode )
		{
			...
			if ( m_currentEvent.keyCode == KeyCode.F5 )
			{
				Debug.Log( "Refreshing available nodes" );
				RefreshAvaibleNodes();
			}
			...
			if ( OnKeyboardPress( KeyCode.F10 ) )
			{
				UIUtils.ForceExampleShaderCompilation();
			}

			if ( OnKeyboardPress( KeyCode.F11 ) )
			{
				m_paletteWindow.DumpAvailableNodes( false );
				m_paletteWindow.DumpAvailableNodes( true );
			}

			if ( OnKeyboardPress( KeyCode.F12 ) )
			{
				m_nodeExporterUtils.ActivateAutoScreenShot();
			}
		}
							...
							case IOUtils.WireConnectionParam:
							{
								...
								if ( inNode != null && outNode != null )
								{
									...
									if ( inputPort != null && outputPort != null )
									{
										...
										else if ( Constants.DeveloperMode )
										{
											if ( !inputCompatible )
	...
	void OnGUI()
	{
		...
		if ( m_forcingShaderUpdateFlag )
		{
			Focus();
			Debug.Log( Event.current.type );
			if ( m_shadersToUpdate.Count > 0 )
			{
				float percentage = 100.0f * ( float ) ( UIUtils.TotalExampleShaders - m_shadersToUpdate.Count ) / ( float ) UIUtils.TotalExampleShaders;
				if ( m_forcingShaderUpdateOp ) // Read
				{
					Debug.Log( percentage + "% Recompiling " + m_shadersToUpdate[ 0 ].name );
					LoadDroppedObject( true, m_shadersToUpdate[ 0 ], null );
				}
				else // Write
				{
					Debug.Log( percentage + "% Saving " + m_shadersToUpdate[ 0 ].name );
					SaveToDisk( false );
					m_shadersToUpdate.RemoveAt( 0 );
				}
				m_forcingShaderUpdateOp = !m_forcingShaderUpdateOp;
			}
			else
			{
				Debug.Log( "100% - All shaders compiled " );
				m_forcingShaderUpdateFlag = false;
			}
		}
		...
		m_repaintIsDirty = m_nodeParametersWindow.Draw( m_cameraInfo, selectedNode, m_currentMousePos2D, m_currentEvent.button ) || m_repaintIsDirty; //TODO: If multiple nodes from the same type are selected also show a parameters window which modifies all of them 
	...
	public void ForceShadersToUpdate( ref Dictionary<string, string> availableShaders )
	{
		m_forcingShaderUpdateOp = true;
		m_forcingShaderUpdateFlag = true;
		m_shadersToUpdate.Clear();
		foreach ( KeyValuePair<string, string> kvp in availableShaders )
		{
			Shader shader = AssetDatabase.LoadAssetAtPath<Shader>( AssetDatabase.GUIDToAssetPath( kvp.Value ) );
			if ( shader != null )
			{
				m_shadersToUpdate.Add( shader );
			}
		}
	}
	...
	public bool ExpandedMain
	{
		get { return m_expandedMain; }
		set { m_expandedMain = value; }
	}

	public bool ExpandedBlend
	{
		get { return m_expandedBlend; }
		set { m_expandedBlend = value; }
	}
	...
	public bool ExpandedRendering
	{
		get { return m_expandedRendering; }
		set { m_expandedRendering = value; }
	}
