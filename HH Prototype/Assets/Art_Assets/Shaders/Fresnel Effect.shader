...
////// Specular:
                ...
                float3 diffuseColor = (_Multiply*(_MainTex_var.rgb*_ColorTint.rgb)); // Need this for specular when using metallic
...
////// Emissive:
                float3 emissive = (_FresnelAmount*(_FresnelColour.rgb*pow(1.0-max(0,dot(normalDirection, viewDirection)),_FresnelExponent)));
/// Final Color:
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                o.Emission = (_FresnelAmount*(_FresnelColour.rgb*pow(1.0-max(0,dot(normalDirection, viewDirection)),_FresnelExponent)));
                
                float4 _MainTex_var = tex2D(_MainTex,TRANSFORM_TEX(i.uv0, _MainTex));
                float3 diffColor = (_Multiply*(_MainTex_var.rgb*_ColorTint.rgb));
                float specularMonochrome;
                float3 specColor;
