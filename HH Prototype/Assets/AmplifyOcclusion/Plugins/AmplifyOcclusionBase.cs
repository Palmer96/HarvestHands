...
public class AmplifyOcclusionBase : MonoBehaviour
{
	...
	[Range( 0, 1 )] public float FadeToIntensity = 1.0f;
	[Range( 0, 16 )] public float FadeToRadius = 1.0f;
	[Range( 0, 16 )] public float FadeToPowerExponent = 1.8f;
	...
	bool CheckRenderTextureFormats()
	{
	...
	void CommandBuffer_FillComputeOcclusion( CommandBuffer cb, TargetDesc target )
	{
		...
		if ( Downsample )
			smallOcclusionRT = SafeAllocateTemporaryRT( cb, "_AO_SmallOcclusionTexture", target.width / 2, target.height / 2, m_occlusionRTFormat, RenderTextureReadWrite.Linear, FilterMode.Bilinear );

		// Ambient Occlusion
		if ( CacheAware && !Downsample )
		{
			int occlusionAtlasRT = SafeAllocateTemporaryRT( cb, "_AO_OcclusionAtlas", target.width, target.height, m_occlusionRTFormat, RenderTextureReadWrite.Linear );
			for ( int i = 0; i < 16; i++ )
			{
		...
		if ( BlurEnabled )
		{
			int tempRT = SafeAllocateTemporaryRT( cb, "_AO_TEMP", target.width, target.height, m_occlusionRTFormat, RenderTextureReadWrite.Linear );
