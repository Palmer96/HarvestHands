	...
	CGINCLUDE
		...
		float3 FetchPosition( float2 uv, bool reinterleave )
		{
			...
			if ( reinterleave )
				...
				viewDepth = LinearEyeDepth( SAMPLE_DEPTH_TEXTURE_LOD( _CameraDepthTexture, float4( uv * _AO_Buffer_PadScale.xy, 0, 0 ) ) );
		...
		{
			return LinearEyeDepth( SAMPLE_DEPTH_TEXTURE_LOD( _CameraDepthTexture, float4( IN.uv, 0, 0 ) ) ).xxxx;
		}
		...
		float4 DeinterleaveDepth1( v2f_img IN ) : SV_Target
		{
			...
			return LinearEyeDepth( SAMPLE_DEPTH_TEXTURE_LOD( _CameraDepthTexture, float4( uv * _AO_Buffer_PadScale.xy, 0, 0 ) ) );
		}
		...
		DepthOutput4 DeinterleaveDepth4( v2f_img IN )
		{
			...
			DepthOutput4 OUT;
			OUT.depth1 = LinearEyeDepth( SAMPLE_DEPTH_TEXTURE_LOD( _CameraDepthTexture, float4( uv1, 0, 0 ) ) ).xxxx;
			OUT.depth2 = LinearEyeDepth( SAMPLE_DEPTH_TEXTURE_LOD( _CameraDepthTexture, float4( uv2, 0, 0 ) ) ).xxxx;
			OUT.depth3 = LinearEyeDepth( SAMPLE_DEPTH_TEXTURE_LOD( _CameraDepthTexture, float4( uv3, 0, 0 ) ) ).xxxx;
			OUT.depth0 = LinearEyeDepth( SAMPLE_DEPTH_TEXTURE_LOD( _CameraDepthTexture, float4( uv0, 0, 0 ) ) ).xxxx;
			return OUT;
		}
		...
		half4 CombineDownsampledOcclusionDepth( v2f_img IN )
		{
			...
			float depth = LinearEyeDepth( SAMPLE_DEPTH_TEXTURE_LOD( _CameraDepthTexture, float4( IN.uv, 0, 0 ) ) );
